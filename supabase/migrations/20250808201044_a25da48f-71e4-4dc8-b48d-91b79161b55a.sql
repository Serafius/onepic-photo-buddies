-- Finalize migration with dropping views before recreation
-- 0) Drop views to avoid column mismatch errors
drop view if exists public.v_portfolio_images;
drop view if exists public.v_portfolio_posts;
...
create view public.v_portfolio_posts as
select 
  pp.*, 
