...
-- Anyone can add comments (no auth requirement for now)
...
-- Anyone can like a comment
create policy if not exists "Anyone can insert comment likes"
  on public.comment_likes
  for insert
  with check (true);

-- Anyone can unlike a comment (delete their like)
create policy if not exists "Anyone can delete comment likes"
  on public.comment_likes
  for delete
  using (true);
