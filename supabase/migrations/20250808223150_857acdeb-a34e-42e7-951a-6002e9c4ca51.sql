...
-- Seed randomized likes for each image with a non-zero base
-- This creates between 7 and 38 likes per image, skipping duplicates if re-run
WITH imgs AS (
...
comments_gen AS (
  SELECT 
    i.id AS image_id,
    (gen_random_uuid())::text AS author_user_id,
